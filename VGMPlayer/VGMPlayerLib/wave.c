...
FILE *rawFilePtr = 0;
int waveCreateWave(const char* filename)
{
	rawFilePtr = fopen(filename, "wb");
	...
	if(rawFilePtr)
		...
		return -1;
	/*
	if (WaveCreateFile(filename, &MMIOOut, &MainWfx, &CkOut, &CkRIFF))
	{
...
int waveWriteFile(int lenght, unsigned char *Buf_Tmp)
{
	fwrite(Buf_Tmp, 1, lenght, rawFilePtr);

	return 0;
	/*
	unsigned int Writted;
...
)
{
	...
	do
	{
		...
		strcat(Name, ext);
	} while (mmioOpen(pszFileName, NULL, MMIO_EXIST) == (HMMIO)TRUE);
	*/
