...
#ifdef ENABLE_ALL_CORES
	case EC_MAME:
		ymf262_update_one(info->chip, DUMMYBUF, 0, DUMMY_CHANNEL_BUF, 0);
		break;
#endif
	case EC_DBOPL:
		adlib_OPL3_getsample(info->chip, DUMMYBUF, 0, DUMMY_CHANNEL_BUF, 0);
		break;
	}
}
...
int device_start_ymf262(UINT8 ChipID, int clock, UINT8 CHIP_SAMPLING_MODE, INT32 CHIP_SAMPLE_RATE, UINT32 SampleRate)
{
