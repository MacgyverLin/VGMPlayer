...
{
public:
	VGMDataImpl(const char* path_, const char* texturePath_, s32 channels_, s32 bitPerSample_, s32 sampleRate_)
	...
	VGMInfo vgmInfo;
	VGMOutputChannels vgmOutputChannels;
};

VGMData::VGMData(const char* path_, const char* texturePath_, s32 channels_, s32 bitPerSample_, s32 sampleRate_)
	: Obserable()
{
...
boolean VGMData::Open()
{
	assert(impl);
	VGMInfo& vgmInfo = impl->vgmInfo;
	VGM_HEADER& vgmHeader = vgmInfo.VGMHead;
	gzFile& hFile = impl->gzFile;

	boolean isvgz = vgmInfo.vgmPath.find(".vgz") != string::npos;
	if (!isvgz)
	{
		return FALSE;
	}

	hFile = gzopen(vgmInfo.vgmPath.c_str(), "r");
	if (!hFile)
		return FALSE;

	// OpenVGMFile_Internal(gzFile hFile, UINT32 FileSize)
	UINT32 fccHeader;
	UINT32 CurPos;
	UINT32 HdrLimit;
	...
	vgmInfo.VGMDataLen = 0;// FileSize;

	gzseek(hFile, 0x00, SEEK_SET);
	//gzrewind(hFile);
	ReadVGMHeader(hFile, &vgmInfo.VGMHead);
	...
	// Read GD3 Tag
	HdrLimit = ReadGD3Tag(impl->gzFile, vgmInfo.VGMHead.lngGD3Offset, &vgmInfo.VGMTag);
	if (HdrLimit == 0x10)
	{
	...
	//impl->vgmOutputChannels.SetChannelsCount(channelsCount);
	return true;
}
...
void VGMData::Close()
{
	...
	free(vgmInfo.VGMH_Extra.Clocks.CCData);
	vgmInfo.VGMH_Extra.Clocks.CCData = NULL;

	free(vgmInfo.VGMH_Extra.Volumes.CCData);
	vgmInfo.VGMH_Extra.Volumes.CCData = NULL;
...
void VGMData::Play()
{
	...
	vgmInfo.IsVGMInit = true;
	vgmInfo.Interpreting = false;
	InterpretFile(0);
	vgmInfo.IsVGMInit = false;
...
boolean VGMData::IsPlaying()
{
	...
	return vgmInfo.PlayingMode != 0xFF;
}
...
boolean VGMData::Update()
{
	...
	int updateSampleCounts = impl->vgmOutputChannels.BeginUpdateSamples(VGM_SAMPLE_BUFFER_SIZE);

	// currentChannel = impl->vgmOutputChannels.HandleUpdateSamples(YM2612_Update, YM2612_GetChannelCount, 0, currentChannel, updateSampleCounts);
	...
	//bufferedLength = FillBuffer(sampleBuffer, updateSampleCounts);
	...
	*/
	impl->vgmOutputChannels.EndUpdateSamples(updateSampleCounts);

	NotifyUpdate();
	impl->vgmOutputChannels.ClearSampleBufferUpdatedEvent();
...
UINT32 VGMData::FillBuffer(WAVE_16BS* Buffer, UINT32 BufferSize)
{
	...
	vgmInfo.CurChipList = (vgmInfo.VGMEnd || vgmInfo.PausePlay) ? 
		vgmInfo.ChipListPause : vgmInfo.ChipListAll;
...
f32 VGMData::GetFrameCounter() const
{
	...
	return vgmInfo.frameCounter;
}

/// <summary>
/// Pause Resume Core function
/// </summary>
/// <param name="Pause"></param>
void VGMData::PauseResume(bool Pause)
{
	assert(impl);
	VGMInfo& vgmInfo = impl->vgmInfo;
...
void VGMData::GeneralChipLists(void)
{
	assert(impl);
	VGMInfo& vgmInfo = impl->vgmInfo;
	VGM_HEADER& VGMHead = vgmInfo.VGMHead;
...
void VGMData::ym2413opl_set_emu_core(UINT8 Emulator)
{
	assert(impl);
	VGMInfo& vgmInfo = impl->vgmInfo;

	vgmInfo.YM2413_EMU_CORE = (Emulator < 0x02) ? Emulator : 0x00;
...
wchar_t* VGMData::ReadWStrFromFile(gzFile hFile, UINT32* FilePos, UINT32 EOFPos)
{
...
UINT8 VGMData::ReadGD3Tag(gzFile hFile, UINT32 GD3Offset, GD3_TAG* RetGD3Tag)
{
	assert(impl);
	VGMInfo& vgmInfo = impl->vgmInfo;
	VGM_HEADER& vgmHeader = vgmInfo.VGMHead;
	...
	else
	{
		...
		CurPos += gzread(hFile, &RetGD3Tag->lngVersion, 4);
		CurPos += gzread(hFile, &RetGD3Tag->lngTagLength, 4);
...
void VGMData::ReadVGMHeader(gzFile hFile, VGM_HEADER* RetVGMHead)
{
	VGM_HEADER CurHead;
	UINT32 CurPos;
	UINT32 HdrLimit;
...
void VGMData::InterpretFile(UINT32 SampleCount)
{
	...
	if (vgmInfo.DacCtrlUsed && SampleCount > 1)	// handle skipping
	{
		...
		{
			daccontrol_update(vgmInfo.DacCtrlUsg[CurChip], SampleCount - 1);
		}
	}
	...
	if (vgmInfo.DacCtrlUsed && SampleCount)
	{
		...
		for (CurChip = 0x00; CurChip < vgmInfo.DacCtrlUsed; CurChip++)
		{
			daccontrol_update(vgmInfo.DacCtrlUsg[CurChip], 1);
		}
	}

	if (vgmInfo.UseFM && vgmInfo.FadePlay)
	{
		//TempLng = PlayingTime % (vgmInfo.SampleRate  / 5);
		//if (! TempLng)
...
void VGMData::AddPCMData(UINT8 Type, UINT32 DataSize, const UINT8* Data)
{
	...
	if (BankSize != TempBnk->DataSize)
		vgm_log("Error reading Data Block! Data Size conflict!\n");
	...
	while (vgmInfo.VGMSmplPos <= SmplPlayed)
	{
		...
		else
		{
			...
			case 0x90:	// DAC Ctrl: Setup Chip
				...
				{
					device_start_daccontrol(CurChip, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
...
void VGMData::Chips_GeneralActions(UINT8 Mode)
{
	...
	case 0x00:	// Start Chips
		...
		if (VGMHead.lngHzYM2413)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				if (!vgmInfo.UseFM)
				{
					CAA->SmpRate = device_start_ym2413(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzYM2612)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_ym2612(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzYM2151)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_ym2151(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzSPCM)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_segapcm(CurChip, ChipClk, VGMHead.lngSPCMIntf, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzRF5C68)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_rf5c68(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzYM2203)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				CAA->SmpRate = device_start_ym2203(CurChip, ChipClk, COpt->SpecialFlags & 0x01,
					...
					vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
				CAA->StreamUpdate = &ym2203_stream_update;
				CAA->Paired->StreamUpdate = &ym2203_stream_update_ay;
				ym2203_set_srchg_cb(CurChip, VGMData::ChangeChipSampleRate, CAA, CAA->Paired);
		...
		if (VGMHead.lngHzYM2608)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ym2608_set_srchg_cb(CurChip, VGMData::ChangeChipSampleRate, CAA, CAA->Paired);
		...
		if (VGMHead.lngHzYM3812)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				if (!vgmInfo.UseFM)
				{
					CAA->SmpRate = device_start_ym3812(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzYM3526)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				if (!vgmInfo.UseFM)
				{
					CAA->SmpRate = device_start_ym3526(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzY8950)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				if (!vgmInfo.UseFM)
				{
					CAA->SmpRate = device_start_y8950(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzYMF262)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				if (!vgmInfo.UseFM)
				{
					CAA->SmpRate = device_start_ymf262(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzYMF278B)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_ymf278b(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzYMF271)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_ymf271(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzYMZ280B)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_ymz280b(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzRF5C164)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_rf5c164(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzPWM)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_pwm(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				if (!vgmInfo.UseFM)
				{
					CAA->SmpRate = device_start_ayxx(CurChip, ChipClk,
						VGMHead.bytAYType, VGMHead.bytAYFlag, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzGBDMG)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_gameboy_sound(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzNESAPU)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_nes(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzMultiPCM)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_multipcm(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzUPD7759)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_upd7759(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzOKIM6258)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				CAA->SmpRate = device_start_okim6258(CurChip, ChipClk,
					...
					(VGMHead.bytOKI6258Flags & 0x08) >> 3, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
				CAA->StreamUpdate = &okim6258_update;

				okim6258_set_srchg_cb(CurChip, VGMData::ChangeChipSampleRate, CAA);
		...
		if (VGMHead.lngHzOKIM6295)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_okim6295(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
				CAA->StreamUpdate = &okim6295_update;
				okim6295_set_srchg_cb(CurChip, VGMData::ChangeChipSampleRate, CAA);
		...
		if (VGMHead.lngHzK051649)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_k051649(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzK054539)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_k054539(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzC140)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_c140(CurChip, ChipClk, VGMHead.bytC140Type, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzK053260)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_k053260(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzPokey)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_pokey(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzQSound)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_qsound(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzSCSP)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_scsp(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzWSwan)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = ws_audio_init(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzVSU)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_vsu(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzSAA1099)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_saa1099(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzES5503)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_es5503(CurChip, ChipClk, VGMHead.bytES5503Chns, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
				CAA->StreamUpdate = &es5503_pcm_update;
				es5503_set_srchg_cb(CurChip, VGMData::ChangeChipSampleRate, CAA);
		...
		if (VGMHead.lngHzES5506)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_es5506(CurChip, ChipClk, VGMHead.bytES5506Chns, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
				CAA->StreamUpdate = &es5506_update;
				es5506_set_srchg_cb(CurChip, VGMData::ChangeChipSampleRate, CAA);
		...
		if (VGMHead.lngHzX1_010)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_x1_010(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzC352)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_c352(CurChip, ChipClk, VGMHead.bytC352ClkDiv * 4, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		if (VGMHead.lngHzGA20)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip++)
			{
				...
				ChipClk = GetChipClock(&VGMHead, (CurChip << 7) | CAA->ChipType, NULL);
				CAA->SmpRate = device_start_iremga20(CurChip, ChipClk, vgmInfo.CHIP_SAMPLING_MODE, vgmInfo.CHIP_SAMPLE_RATE);
		...
		//memset(vgmInfo.vgmInfo.DacCtrl, 0x00, sizeof(DACCTRL_DATA) * 0xFF);
	...
	case 0x01:	// Reset chips
		...
		for (CurChip = 0x00; CurChip < vgmInfo.DacCtrlUsed; CurChip++)
		{
			...
			//vgmInfo.vgmInfo.DacCtrl[CurCSet].Enable = false;
		}
		//vgmInfo.DacCtrlUsed = 0x00;
		//memset(DacCtrlUsg, 0x00, 0x01 * 0xFF);
...
void VGMData::chip_reg_write(UINT8 ChipType, UINT8 ChipID,
...
#if 0
	updateSampleCounts = impl->vgmOutputChannels.BeginUpdateSamples(updateSampleCounts);

	int currentChannel = 0;
	if (impl->vgmHeader.YM2612Clock)
	{
		currentChannel = impl->vgmOutputChannels.HandleUpdateSamples(YM2612_Update, YM2612_GetChannelCount, 0, currentChannel, updateSampleCounts);
	}
	if (impl->vgmHeader.SN76489Clock)
	{
		currentChannel = impl->vgmOutputChannels.HandleUpdateSamples(SN76489_Update, SN76489_GetChannelCount, 0, currentChannel, updateSampleCounts);
	}
	if (impl->vgmHeader.YM2151Clock)
	{
		currentChannel = impl->vgmOutputChannels.HandleUpdateSamples(YM2151_Update, YM2151_GetChannelCount, 0, currentChannel, updateSampleCounts);
	}
	if (impl->vgmHeader.K053260Clock)
	{
		currentChannel = impl->vgmOutputChannels.HandleUpdateSamples(K053260_Update, K053260_GetChannelCount, 0, currentChannel, updateSampleCounts);
	}
	if (impl->vgmHeader.NESAPUClock)
	{
		currentChannel = impl->vgmOutputChannels.HandleUpdateSamples(NESAPU_Update, NESAPU_GetChannelCount, 0, currentChannel, updateSampleCounts);
	}
	if (impl->vgmHeader.NESAPUClock & 0x80000000)
	{
		currentChannel = impl->vgmOutputChannels.HandleUpdateSamples(NESFDSAPU_Update, NESFDSAPU_GetChannelCount, 0, currentChannel, updateSampleCounts);
	}
	if (impl->vgmHeader.HuC6280Clock)
	{
		currentChannel = impl->vgmOutputChannels.HandleUpdateSamples(HUC6280_Update, HUC6280_GetChannelCount, 0, currentChannel, updateSampleCounts);
	}
	if (impl->vgmHeader.QSoundClock)
	{
		currentChannel = impl->vgmOutputChannels.HandleUpdateSamples(QSound_Update, QSound_GetChannelCount, 0, currentChannel, updateSampleCounts);
	}
	if (impl->vgmHeader.SegaPCMclock)
	{
		currentChannel = impl->vgmOutputChannels.HandleUpdateSamples(SEGAPCM_Update, SEGAPCM_GetChannelCount, 0, currentChannel, updateSampleCounts);
	}
