...
#include <float.h>
static float minv = FLT_MAX;
static float maxv = FLT_MIN;
...
typedef struct
{
	...
	u8			tim_A;					/* timer A enable (0-disabled) */
	u8			tim_B;					/* timer B enable (0-disabled) */
	s32			tim_A_val;				/* current value of timer A */
	s32			tim_B_val;				/* current value of timer B */
	u32			tim_A_tab[1024];		/* timer A deltas */
	u32			tim_B_tab[256];			/* timer B deltas */
	...
	// void(*irqhandler)(s32 irq);		/* IRQ function handler */
...
*	0     600   781   950
*/
static const u32 dt2_tab[4] = { 0, 384, 500, 608 };
...
static const u8 dt1_tab[4 * 32] = { /* 4*32 DT1 values */
								 /* DT1=0 */
...
#define YM2151_CHIPS_COUNT 2
YM2151 ym2151Chips[YM2151_CHIPS_COUNT];	/* array of YM2151's */
...
void init_chip_tables(YM2151* ic)
{
	s32 i, j;
	f32 mult, pom, phaseinc, Hz;
	f32 scaler;

	scaler = ((f64)ic->clock / 64.0) / ((f64)ic->sampfreq);
	...
	for (i = 0; i < 768; i++)
	{
		...
		ic->freq[768 + 2 * 768 + i] = ((s32)(phaseinc * mult)) & 0xffffffc0; /* adjust to X.10 fixed point */
																			 /* octave 0 and octave 1 */
		for (j = 0; j < 2; j++)
		{
			ic->freq[768 + j * 768 + i] = (ic->freq[768 + 2 * 768 + i] >> (2 - j)) & 0xffffffc0; /* adjust to X.10 fixed point */
		}
		/* octave 3 to 7 */
		for (j = 3; j < 8; j++)
		{
			ic->freq[768 + j * 768 + i] = ic->freq[768 + 2 * 768 + i] << (j - 2);
		}
	}
	...
	for (i = 0; i < 768; i++)
	{
		ic->freq[0 * 768 + i] = ic->freq[1 * 768 + 0];
	}
	...
	for (j = 8; j < 10; j++)
	{
		for (i = 0; i < 768; i++)
		{
			ic->freq[768 + j * 768 + i] = ic->freq[768 + 8 * 768 - 1];
		}
	}
	...
	for (j = 0; j < 4; j++)
	{
		for (i = 0; i < 32; i++)
		{
			Hz = ((f64)dt1_tab[j * 32 + i] * ((f64)ic->clock / 64.0)) / (f64)(1 << 20);

			/*calculate phase increment*/
			phaseinc = (Hz * SIN_LEN) / (f64)ic->sampfreq;

			/*positive and negative values*/
			ic->dt1_freq[(j + 0) * 32 + i] = phaseinc * mult;
			ic->dt1_freq[(j + 4) * 32 + i] = -ic->dt1_freq[(j + 0) * 32 + i];
		}
	}
	...
	for (i = 0; i < 1024; i++)
	{
		...
		pom = (64.0 * (1024.0 - i) / (f64)ic->clock);
		ic->tim_A_tab[i] = pom * (f64)ic->sampfreq * mult;  /* number of samples that timer period takes (fixed point) */
	}
	for (i = 0; i < 256; i++)
	{
		/* ASG 980324: changed to compute both tim_B_tab and timer_B_time */
		pom = (1024.0 * (256.0 - i) / (f64)ic->clock);
		ic->tim_B_tab[i] = pom * (f64)ic->sampfreq * mult;  /* number of samples that timer period takes (fixed point) */
	}
	...
	for (i = 0; i < 32; i++)
	{
		...
		j = 32 - j;
		j = (65536.0 / (f64)(j * 32.0));	/* number of samples per one shift of the shift register */
											/*ic->noise_tab[i] = j * 64;*/	/* number of chip clock cycles per one shift */
		ic->noise_tab[i] = j * 64 * scaler;
...
#define KEY_ON(op, key_set){									\
		...
		{														\
			...
			(op)->volume += (~(op)->volume *					\
                           (eg_inc[(op)->eg_sel_ar + ((ic->eg_cnt>>(op)->eg_sh_ar)&7)])	\
                          ) >>4;								\
...
void envelope_KONKOFF(YM2151* ic, int ch, YM2151Operator* op, s32 v, s32* channel, f32* freq)
{
	if ((ic->channel_output_enabled & (1 << (ch))))
	{
		*channel = ch;
		*freq = (op + 0)->freq * 879.91f / 1340160;
	}
...
void set_connect(YM2151* ic, YM2151Operator* om1, s32 cha, s32 v)
{
...
s32 op_calc(YM2151Operator* OP, u32 env, s32 pm)
{
	u32 p;
...
s32 op_calc1(YM2151Operator* OP, u32 env, s32 pm)
{
	u32 p;
	s32  i;
...
void chan_calc(YM2151* ic, u32 chan)
{
	YM2151Operator* op;
	u32 env;
	u32 AM = 0;
...
void advance_eg(YM2151* ic)
{
	YM2151Operator* op;
	u32 i;

	ic->eg_timer += ic->eg_timer_add;

	while (ic->eg_timer >= ic->eg_timer_overflow)
	{
		ic->eg_timer -= ic->eg_timer_overflow;

		ic->eg_cnt++;

		/* envelope generator */
		op = &ic->oper[0];	/* CH 0 M1 */
		i = 32;
		do
		{
			switch (op->state)
			...
			case EG_DEC:	/* decay phase */
				if (!(ic->eg_cnt & ((1 << op->eg_sh_d1r) - 1)))
				{
					op->volume += eg_inc[op->eg_sel_d1r + ((ic->eg_cnt >> op->eg_sh_d1r) & 7)];
			...
			case EG_SUS:	/* sustain phase */
				if (!(ic->eg_cnt & ((1 << op->eg_sh_d2r) - 1)))
				{
					op->volume += eg_inc[op->eg_sel_d2r + ((ic->eg_cnt >> op->eg_sh_d2r) & 7)];
			...
			case EG_REL:	/* release phase */
				if (!(ic->eg_cnt & ((1 << op->eg_sh_rr) - 1)))
				{
					op->volume += eg_inc[op->eg_sel_rr + ((ic->eg_cnt >> op->eg_sh_rr) & 7)];
...
void advance(YM2151* ic)
{
	...
	{
		ic->lfo_timer += ic->lfo_timer_add;
		if (ic->lfo_timer >= ic->lfo_overflow)
		{
			ic->lfo_timer -= ic->lfo_overflow;
			ic->lfo_counter += ic->lfo_counter_add;
			ic->lfo_phase += (ic->lfo_counter >> 4);
			ic->lfo_phase &= 255;
			ic->lfo_counter &= 15;
		}
	}
	...
	*/
	ic->noise_p += ic->noise_f;
	i = (ic->noise_p >> 16);		/* number of events (shifts of the shift register) */
	ic->noise_p &= 0xffff;
	while (i)
	{
		u32 j;
		j = ((ic->noise_rng ^ (ic->noise_rng >> 3)) & 1) ^ 1;
		ic->noise_rng = (j << 16) | (ic->noise_rng >> 1);
		i--;
	}
	...
	do
	{
		...
		{
			s32 mod_ind = ic->lfp;		/* -128..+127 (8bits signed) */
			if (op->pms < 6)
				mod_ind >>= (6 - op->pms);
			else
				mod_ind <<= (op->pms - 5);
	...
	*/
	...
	if (ic->csm_req)			/* CSM KEYON/KEYOFF seqeunce request */
	{
		if (ic->csm_req == 2)	/* KEY ON */
		{
			op = &ic->oper[0];	/* CH 0 M1 */
			i = 32;
			do
...
void YM2151_WriteRegister(u8 chipID, u32 r, u32 v, s32* channel, f32* freq)
{
	...
	case 0x00:
		...
		case 0x08:
			envelope_KONKOFF(ic, ch, &ic->oper[(v & 7) * 4], v, channel, freq);
			break;
...
void YM2151_Update(u8 chipID, s32** bufs, u32 length)
{
	...
	if (ic->tim_B)
	{
		ic->tim_B_val -= (length << TIMER_SH);
		if (ic->tim_B_val <= 0)
		{
			ic->tim_B_val += ic->tim_B_tab[ic->timer_B_index];
			if (ic->irq_enable & 0x08)
			{
				s32 oldstate = ic->status & 3;
				ic->status |= 2;
				/*
				if ((!oldstate) && (ic->irqhandler))
					(*ic->irqhandler)(1);
				*/
			}
		}
	...
	for (sample = 0; sample < length; sample++)
	{
		...
		SAVE_ALL_CHANNELS()
			...
			if (ic->tim_A)
			{
				ic->tim_A_val -= (1 << TIMER_SH);
				if (ic->tim_A_val <= 0)
				{
					ic->tim_A_val += ic->tim_A_tab[ic->timer_A_index];
					if (ic->irq_enable & 0x04)
					{
						s32 oldstate = ic->status & 3;
						ic->status |= 1;
						/*
					...
					if (ic->irq_enable & 0x80)
						ic->csm_req = 2;	/* request KEY ON / KEY OFF sequence */
				}
			}
