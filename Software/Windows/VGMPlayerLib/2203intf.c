...
void ym2203_stream_update(UINT8 ChipID, stream_sample_t **outputs, int samples, WAVE_32BS** channeloutputs, int channelcount)
{
	...
	ym2203_update_one(info->chip, outputs, samples, channeloutputs, channelcount);
}
	...
	if (! AYDisable)
	{
		...
		*AYrate = ay_clock / 8;
		...
		case EC_MAME:
			...
			info->psg = ay8910_start_ym(NULL, CHTYPE_YM2203, ay_clock, &intf->ay8910_intf, CHIP_SAMPLING_MODE, CHIP_SAMPLE_RATE, SampleRate);
			break;
#endif
		case EC_EMU2149:
