...
void ym2612_stream_update(UINT8 ChipID, stream_sample_t **outputs, int samples, WAVE_32BS** channeloutputs, int channelcount)
{
	//ym2612_state *info = (ym2612_state *)param;
	...
	{
	case EC_MAME:
		ym2612_update_one(info->chip, outputs, samples, channeloutputs, channelcount);
		break;
#ifdef ENABLE_ALL_CORES
	case EC_GENS:
		YM2612_ClearBuffer(GensBuf, samples, channeloutputs, channelcount);
		YM2612_Update(info->chip, GensBuf, samples, channeloutputs, channelcount);
		YM2612_DacAndTimers_Update(info->chip, GensBuf, samples, channeloutputs, channelcount);
		...
		break;
	case EC_NUKED:
		OPN2_GenerateStream(info->chip, outputs, samples, channeloutputs, channelcount);
		break;
#endif
	}
}
...
int device_start_ym2612(UINT8 ChipID, int clock, UINT8 CHIP_SAMPLING_MODE, INT32 CHIP_SAMPLE_RATE, UINT32 SampleRate)
{
