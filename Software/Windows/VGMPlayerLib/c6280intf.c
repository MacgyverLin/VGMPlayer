...
void c6280_update(UINT8 ChipID, stream_sample_t **outputs, int samples, WAVE_32BS** channeloutputs, int channelcount)
{
...
#ifdef ENABLE_ALL_CORES
	case EC_MAME:
		c6280m_update(info->chip, outputs, samples, channeloutputs, channelcount);
		break;
#endif
	case EC_OOTAKE:
		PSG_Mix(info->chip, outputs, samples, channeloutputs, channelcount);
		break;
	}
}
...
int device_start_c6280(UINT8 ChipID, int clock, UINT8 CHIP_SAMPLING_MODE, INT32 CHIP_SAMPLE_RATE, UINT32 SampleRate)
{
	...
	case EC_MAME:
		...
		info->chip = device_start_c6280m(clock, rate, CHIP_SAMPLING_MODE, CHIP_SAMPLE_RATE, SampleRate);
		if (info->chip == NULL)
			return 0;
		break;
#endif
