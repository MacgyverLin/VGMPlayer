...
void ADLIBEMU(getsample)(void* chip, INT32** sndptr, INT32 numsamples, WAVE_32BS** channeloutputs, INT32 channelcount)
{
	OPL_DATA* OPL = (OPL_DATA*)chip;
		...
		for (cur_ch = max_channel - 1; cur_ch >= 0; cur_ch--)
		{
			...
			else
			{
				...
				for (i = 0; i < endsamples; i++)
				{
					...
					channeloutputs[cur_ch][i].Left = chanval;
					channeloutputs[cur_ch][i].Right = chanval;
					CHANVAL_OUT(0)
				}
			}
		}
