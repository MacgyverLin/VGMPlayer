...
int device_start_qsound(UINT8 ChipID, int clock, UINT8 CHIP_SAMPLING_MODE, INT32 CHIP_SAMPLE_RATE, UINT32 SampleRate)
{
...
#ifdef ENABLE_ALL_CORES
	case EC_MAME:
		return device_start_qsoundm(ChipID, clock, CHIP_SAMPLING_MODE, CHIP_SAMPLE_RATE, SampleRate);
#endif
	case EC_CTR:
		if(clock < 10000000)
		{
		...
		return device_start_qsound_ctr(ChipID, clock, CHIP_SAMPLING_MODE, CHIP_SAMPLE_RATE, SampleRate);
	}
	return 0;
}
...
void qsound_update(UINT8 ChipID, stream_sample_t **outputs, int samples, WAVE_32BS** channeloutputs, int channelcount)
{
	switch(EMU_CORE)
	{
#ifdef ENABLE_ALL_CORES
	case EC_MAME:
		qsoundm_update(ChipID, outputs, samples, channeloutputs, channelcount); return;
#endif
	case EC_CTR:
		qsoundc_update(ChipID, outputs, samples, channeloutputs, channelcount); return;
	}
}
