...
INLINE void apu_update(nesapu_state *info, stream_sample_t **buffer16, int samples, WAVE_32BS** channeloutputs, int channelcount)
{
	int accum;
...
void nes_psg_update_sound(void* chip, stream_sample_t **outputs, int samples, WAVE_32BS** channeloutputs, int channelcount)
{
	//nesapu_state *info = (nesapu_state *)param;
	nesapu_state *info = (nesapu_state*)chip;
	//apu_update(info, outputs[0], samples);
	apu_update(info, outputs, samples, channeloutputs, channelcount);
}
