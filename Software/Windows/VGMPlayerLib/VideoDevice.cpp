...
GLenum GetGLEnum(VideoDeviceEnum constant)
{
...
, height(0)
, nrComponents(0)
, wrapS(VideoDeviceEnum::REPEAT)
, wrapR(VideoDeviceEnum::REPEAT)
, wrapT(VideoDeviceEnum::REPEAT)
, minFilter(VideoDeviceEnum::LINEAR_MIPMAP_LINEAR)
, magFilter(VideoDeviceEnum::LINEAR)
{
}
...
void Texture2D::Bind() const
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GetGLEnum(magFilter));
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GetGLEnum(minFilter));
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GetGLEnum(wrapS));
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_R, GetGLEnum(wrapR));
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GetGLEnum(wrapT));
}

void Texture2D::SetWrapS(VideoDeviceEnum wrapS_)
{
	wrapS = wrapS_;
}

void Texture2D::SetWrapR(VideoDeviceEnum wrapR_)
{
	wrapR = wrapR_;
}

void Texture2D::SetWrapT(VideoDeviceEnum wrapT_)
{
	wrapT = wrapT_;
}

void Texture2D::SetMinFilter(VideoDeviceEnum minFilter_)
{
	minFilter = minFilter_;
}

void Texture2D::SetMagFilter(VideoDeviceEnum magFilter_)
{
...
void VideoDevice::ReadPixels(void* buffer)
{
	glReadPixels(0, 0, impl->width, impl->height, GL_RGB, GL_UNSIGNED_BYTE, buffer);
}

void VideoDevice::Enable(VideoDeviceEnum cap)
{
	glEnable(GetGLEnum(cap));
}

void VideoDevice::Disable(VideoDeviceEnum cap)
{
...
void VideoDevice::BlendFunc(VideoDeviceEnum sfactor, VideoDeviceEnum dfactor)
{
	glBlendFunc(GetGLEnum(sfactor), GetGLEnum(dfactor));
}

void VideoDevice::MatrixMode(VideoDeviceEnum mode)
{
