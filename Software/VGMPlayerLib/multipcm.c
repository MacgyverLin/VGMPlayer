...
static MultiPCM multiPCMs[MAX_CHIPS];
...
void multipcm_set_bank(u8 ChipID, u32 leftoffs, u32 rightoffs);
...
static void LFO_ComputeStep(MultiPCM* ptChip, struct _LFO* LFO, u32 LFOF, u32 LFOS, int ALFO)
{
...
void MultiPCM_update(u8 ChipID, s32** outputs, int samples)
{
	//MultiPCM *ptChip = (MultiPCM *)param;
	MultiPCM* ptChip = &multiPCMs[ChipID];
	s32* datap[2];
	int i, sl;
...
u8 multipcm_r(u8 ChipID, int offset)
{
...
int device_start_multipcm(u8 ChipID, int clock)
{
	//MultiPCM *ptChip = get_safe_token(device);
	MultiPCM* ptChip;
	int i;
	...
	/*for(i=0;i<512;++i)
	{
		u8 *ptSample=(u8 *) ptChip->ROM+i*12;
	...
	ptChip->channel_output_enabled = 0xffffffff;
	ptChip->channel_count = 1;

	return (int)(ptChip->Rate + 0.5);
}


void device_stop_multipcm(u8 ChipID)
{
	MultiPCM* ptChip = &multiPCMs[ChipID];
...
void device_reset_multipcm(u8 ChipID)
{
	MultiPCM* ptChip = &multiPCMs[ChipID];
	int i;
...
void multipcm_w(u8 ChipID, int offset, u8 data)
{
	//MultiPCM *ptChip = get_safe_token(device);
	MultiPCM* ptChip = &multiPCMs[ChipID];
	switch (offset)
...
void multipcm_bank_write(u8 ChipID, u8 offset, UINT16 data)
{
	MultiPCM* ptChip = &multiPCMs[ChipID];
...
void multipcm_write_rom(u8 ChipID, u32 ROMSize, u32 DataStart, u32 DataLength, const u8* ROMData)
{
	MultiPCM* ptChip = &multiPCMs[ChipID];
	UINT16 CurSmpl;
	...
	if (DataStart < 0x200 * 12)
	{
		for (CurSmpl = 0; CurSmpl < 512; CurSmpl++)
		{
			...
			ptSample = (u8*)ptChip->ROM + CurSmpl * 12;
...
void multipcm_set_mute_mask(u8 ChipID, u32 MuteMask)
{
	MultiPCM* ptChip = &multiPCMs[ChipID];
	u8 CurChn;
...
u8 multipcm_get_channels(u8 ChipID, u32* ChannelMask)
{
	MultiPCM* ptChip = &multiPCMs[ChipID];
	u8 CurChn;
	u8 UsedChns;
...
{
	device_start_multipcm(chipID, clock);
...
{
	MultiPCMUpdate(chipID, buffer, length);
}
