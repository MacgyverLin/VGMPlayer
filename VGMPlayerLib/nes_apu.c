...
static struct nesapu_info nesapu_chips[CHIP_NUM];
...
//------------------------------------------------------------------------------------------------------
	if (info->pSyncCallback == 0) return;
	INT32 position;
...
/* UPDATE APU SYSTEM */
void nesapuUpdate(INT32 chip, INT32** buf, INT32 samples)
{
#if defined FBA_DEBUG
	...
	*/
	...
	for (INT32 i = 0; i < samples; i++) {
		if ((pos>>12) >= info->samples_per_frame) 
			pos = (info->samples_per_frame-1) << 12;
		...
		if (nAdd) {	
			buf[LEFT][i] += nLeftSample;
			buf[RIGHT][i] += nRightSample;
		} else {
			buf[LEFT][i] = nLeftSample;
			buf[RIGHT][i] = nRightSample;
		}
...
//	DebugSnd_NESAPUSndInitted = 1;
	...
	memset(info, 0, sizeof(struct nesapu_info));

	int nBurnFPS = 60; //here
	...
	info->stream = 0;
	info->stream = (INT16*)malloc(info->samples_per_frame * sizeof(INT16));
